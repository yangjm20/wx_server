        ...
        UserInfoModel.findOne({userId: r1.openid}, function (errors, userInfos) {
            if (!userInfos) {
                new UserInfoModel({
                    userId: r1.openid,
                    isMember:false,
                    isBuy: [{lessonIsBuy: false}, {lessonIsBuy: false}, {lessonIsBuy: false}, {lessonIsBuy: false}]
                    ...
                    if (userInfo) {
                        ...
                        IsExercisedModel.findOne({userId: r1.openid}, function (err, isExercise) {
                            if (!isExercise) {
                                ...
                                new IsExercisedModel({
                                    userId: r1.openid,
                                    lessons: lessons['lessons']
                                }).save(function (err, saveIsExe) {
                                    if (saveIsExe) {
    ...
    if (lessonId == "-1") {
        UserInfoModel.findOne({userId}, function (error, userInfo) {
            if (userInfo) {
                ...
                UserInfoModel.update({userId}, {$set: {isMember:true,isBuy: isBuy}}, function (err, userInfoUpdate) {
